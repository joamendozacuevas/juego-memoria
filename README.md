...
Tablero dinamico y barajado: createDeck(), shuffle() y render() crean las cartas desde state.cards al iniciar o reiniciar.
Voltear con click via delegacion: existe un solo listener en #tablero; handleBoardClick() detecta la carta usando event.target.closest(".card").
Comparacion de dos cartas: resolveTurn() compara el pairId de ambas cartas en el estado.
Ocultar tras retardo: cuando no coinciden, setTimeout() espera 800 ms y vuelve a cerrar ambas cartas.
Bloqueo del tablero: state.locked impide voltear una tercera carta, repetir la misma carta o actuar durante la animacion.
Movimientos y cronometro: state.moves, startTimer() y formatTime() actualizan los marcadores.
Victoria: checkVictory() verifica si todas las cartas tienen isFound === true.
Reinicio: el boton Reiniciar llama a resetGame(), vuelve a barajar y limpia movimientos, tiempo y seleccion.
Eventos extra: change cambia la dificultad, keydown reinicia con la tecla R e input actualiza el nombre.
Retroalimentacion visual: las clases is-open e is-found muestran carta volteada y pareja encontrada; #estado comunica victoria o comparacion.
Bonus: dificultad variable y mejor puntaje guardado en localStorage.
