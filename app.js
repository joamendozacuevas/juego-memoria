const SYMBOLS = ["🍎", "🚀", "🐱", "🌵", "🎲", "🎧", "⚽", "🍕"];
const STORAGE_PREFIX = "memoria-mejor-";
...
function createDeck(pairCount) {
  ...
  const deck = chosenSymbols.flatMap((symbol, pairIndex) => [
    createCard(symbol, pairIndex, 1),
    createCard(symbol, pairIndex, 2)
  ]);
...
function updateBestScore() {
  ...
  elements.best.textContent = best ? `${best.moves} mov. / ${formatTime(best.seconds)}` : "--";
}
...
function render() {
  elements.board.style.setProperty("--columns", getColumnCount());
